...
def printResult(lengthFreq,query):
    lengthFreq = sorted(lengthFreq.items(), key=lambda item: item[1], reverse=True)[0:10]
    with open('output.txt', 'w') as f:
        temp = "https://en.wikipedia.org/wiki/{}".format(query)
        f.write(temp)
        f.write("\n")
        for tup in lengthFreq:
            (docId, _) = tup
            cont = docToTitle[docId].replace(" ","_")
            temp = "https://en.wikipedia.org/wiki/{}".format(cont)
            f.write(temp)
        ...
        for word in finalTokens:
            ...
            for pl in postingList:
                ...
                for cf in categoryFreq:
                    ...
                    tf += (freq * weight[cat])
                finalDict[docId] += float(log10(1 + tf)) * float(idf)
    printResult(finalDict,queryText)
